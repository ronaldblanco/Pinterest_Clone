...
(function () {
   ...
   function updateGrid (data) {
      ...
      for(var a = 0; a < wins.length; a++){
         grid.innerHTML = grid.innerHTML + '<div class="grid-item-public"><img src = "'+wins[a].image+'" class = "myimg img-rounded"><p class="txt">'+wins[a].name+'</p><p class="txtuser">By '+wins[a].username+'</p>  <input type="radio" value="?name=' +wins[a].name+'&username='+wins[a].username+ '" name="radioLike" id="radioLike'+a+'">Likes '+wins[a].likes.length+'  <img src = "'+wins[a].userimage+'" class = "allwinsuser img-rounded"></div>'
      }
